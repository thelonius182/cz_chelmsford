...
ent_tbl <- if (cur_build_type == BUILD_TYPE$PROD) "entries" else "clock_revision_entries"
qry <- glue_sql("select id, parent_id, type, title_nl, 
        ...
        where deleted_at is null 
          and site_id = 1
          and type in ('broadcast', 'episode');", .con = con)
...
df_cpnm_broadcasts <- df_cpnm_items |> 
  filter(type == "broadcast" & dttm_start >= his_stop_ts - days(200)) |> 
