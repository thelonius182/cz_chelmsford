...
df_backfill_final <- df_joined_slots_backfill |> left_join(df_linked_episodes, by = join_by(wp_post_id == legacy_id)) |> 
...
for (rn in seq_len(nrow(df_backfill_final))) {
  sprintf(str_glue("chaining {df_backfill_final$episode_chain[rn]} item {rn}"))
  result <- append_chain_item(pm_con = con,
                              pm_label = df_backfill_final$episode_chain[rn],
                              pm_episode_entry_id = df_backfill_final$episode_id[rn],
                              pm_step_completed_last = STEP$PLAYOUT,
