...
repeat {
  ...
  # > add new to database ----
  # do new ones first, to include them in the list of possible replays
  df_new_episodes_fresh <- df_clock_cz_cur.5 |> filter(!is_replay & is.na(episode_entry_id))

  # Let all inserted rows have identical `created_at` timestamps, making it a set. Prep the creation date 
  # in the way the database stores it: timezone UTC
  ts_now = now()
  ...
  if (nrow(df_new_episodes_fresh) > 0) {
    ...
    clock2db_sts <- clock2db(pm_clock_tib = df_new_episodes_fresh, 
  ...
  clock_diffs <- df_clock_cz_cur.7 |> left_join(prod_clock_set_db, by = join_by(slot == bc_start)) |> 
    filter(is.na(bc_name) | bc_name != titel_NL) |> select(slot, bc_id, bc_name, is_replay, prod_ep_has_content = ep_has_content)

  # delete broadcasts in db ----
  # - of replays: always
  # - of fresh episodes: only of those episodes not having any content yet; those with content should remain intact
  diffs_replay_delete <- clock_diffs |> 
    filter(!is.na(bc_id) & (is_replay | !is_replay & (is.na(prod_ep_has_content) | prod_ep_has_content == "N"))) |> 
    transmute(bc_id_delete = bc_id)

  if (nrow(diffs_replay_delete) > 0) {
    sql_sts <- dbExecute(con, "drop temporary table if exists diffs_replay_delete")
    sql_sts <- dbExecute(con, "create temporary table diffs_replay_delete (
                                 bc_id_delete char(36) character set utf8mb4 collate utf8mb4_unicode_ci)")
    sql_sts <- dbAppendTable(con, "diffs_replay_delete", diffs_replay_delete)
    sql_stmt <- glue_sql("update entries as e
                          join diffs_replay_delete as c on c.bc_id_delete = e.id
                          set deleted_at = {fmt_ts(ts_now)}
  ...
  # > add new diffs to database ----
  df_new_episodes_fresh_diffs <- clock_diffs |> 
    filter(is.na(bc_id) & !is_replay & (is.na(prod_ep_has_content) | prod_ep_has_content == "N")) |> 
    select(slot) |> inner_join(df_clock_cz_cur.7, by = join_by(slot))

  if (nrow(df_new_episodes_fresh_diffs) > 0) {
    flog.info(str_glue("replacing fresh clock items to production `entries``, using `created_at` = {fmt_created_at} UTC"), 
              name = "clof")
    clock2db_sts <- clock2db(pm_clock_tib = df_new_episodes_fresh_diffs, 
                             pm_created_at = ts_now, 
                             pm_site = SITE$CONCERTZENDER, 
                             pm_build_type = BUILD_TYPE$PROD,
                             pm_db = con)
    
  }
  
  # > add replay diffs to database ----
  df_new_episodes_replay_diffs <- clock_diffs |> filter(is_replay) |> 
    select(slot) |> inner_join(df_clock_cz_cur.7, by = join_by(slot))
  
  n <- nrow(df_new_episodes_replay_diffs)
  
  if (n > 0) {
    flog.info("adding replacement replays, same `created_at`", name = "clof")
    for (rn in seq_len(n)) {
      episode_entry_id[rn] <- lookup_replay(pm_chains = df_chains,
                                            pm_cur_chain = df_new_episodes_replay_diffs$episode_chain[rn],
                                            pm_replay_target_slot = df_new_episodes_replay_diffs$slot[rn],
                                            pm_bc_type = df_new_episodes_replay_diffs$uitzendtype[rn],
                                            pm_nipperstudio = df_new_episodes_replay_diffs$nipper_mogelijk[rn],
                                            pm_start_of_week = start_of_week(df_new_episodes_replay_diffs$slot[rn]))
      # log as "not found"
      if (episode_entry_id[rn] == "NOT-FOUND") {
        v1 <- df_new_episodes_replay_diffs$titel_NL[rn]
        v2 <- df_new_episodes_replay_diffs$slot[rn]
        v3 = df_new_episodes_replay_diffs$episode_chain[rn]
        flog.error(str_glue("no replay found for {v1}, target slot {v2}, of chain {v3}"), name = "clof")
      } else {
        # add broadcast to the episode to be replayed
        ins_result <- cpnm_bc_ins(pm_pgm_id = df_new_episodes_replay_diffs$pgm_id[rn],
                                  pm_epi_id = episode_entry_id[rn],
                                  pm_site_id = SITE$CONCERTZENDER,
                                  pm_bc_start = df_new_episodes_replay_diffs$slot[rn],
                                  pm_bc_minutes = df_new_episodes_replay_diffs$slot_minutes[rn],
                                  pm_created_at = ts_now,
                                  pm_build_type = BUILD_TYPE$PROD,
                                  pm_cpnm_db = con)
      }
